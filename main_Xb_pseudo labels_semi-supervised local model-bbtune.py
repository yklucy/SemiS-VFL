...
def main():
    ...
    Xb_unaligned_loader = torch.utils.data.DataLoader(torch.tensor(Xb_unaligned, dtype=torch.float), batch_size=args.batch_size, pin_memory=False, drop_last=False)
    ...
    Xb_test_loader = torch.utils.data.DataLoader(Xb_test_wrap, batch_size=args.batch_size, pin_memory=False, drop_last=False)

    ########################################################
    # load models
    if args.dataset == 'NUSWIDE':
        local_model_Xb = model_one(input_shape_Xb, args, dropout_rate=0.9)
    else:
    ...
    if args.model_path !='':
        ...
        local_model_Xb = load_models(local_model_Xb, args.models, 'tune-2-bb', args, use_cross_model=False, use_local_model=True)

    ############################################################
    # generate pseudo labels for Party B unaligned samples
    local_model_Xb.eval()
    if args.dataset == 'NUSWIDE':
    ...
    confident_unaligned_Xb = []
    pseudo_labels = []
    number = 0
    with torch.no_grad():
        for Xb_unaligned in Xb_unaligned_loader:
            if args.dataset == 'EMNIST':
                Xb_unaligned = Xb_unaligned.reshape(-1, 1, 14, 28)
            if args.dataset == 'CIFAR10':
                Xb_unaligned = Xb_unaligned.reshape(-1, 3, 16, 32)

            z_pseudo = local_model_Xb(Xb_unaligned)
            logits_pseudo = local_model_Xb.global_model_classifier_head_single(z_pseudo) 
            probabilities_pseudo = torch.nn.functional.softmax(logits_pseudo, dim=1)
            confident_predictions = probabilities_pseudo.max(dim=1)
            confident_mask = confident_predictions.values > confidence_threshold
            confident_labels = confident_predictions.indices[confident_mask]
            Xb_unaligned_flat = Xb_unaligned.reshape(len(Xb_unaligned),-1)
            Xb_unaligned_confident = Xb_unaligned_flat[confident_mask]

            confident_unaligned_Xb.extend(Xb_unaligned_confident.tolist())
            pseudo_labels.extend(confident_labels.tolist())
            number += len(Xb_unaligned)

    y_aligned = y_aligned.reshape(-1)
    confident_unaligned_Xb = np.array(confident_unaligned_Xb)
    print('  hi  hi   hi   hi   psedudo   ')
    print('len confident_unaligned_Xb:{}'.format(len(confident_unaligned_Xb)))
    pseudo_labels = np.array(pseudo_labels)

    if (len(confident_unaligned_Xb) == 0):
        Xb_aligned_pseudo = Xb_aligned
        Xb_aligned_pseudo_y = y_aligned
    else:
        Xb_aligned_pseudo = np.concatenate((Xb_aligned, confident_unaligned_Xb), axis=0)
        Xb_aligned_pseudo_y = np.concatenate((y_aligned, pseudo_labels), axis=0)
    
    Xb_aligned_pseudo_wrap = list(zip(torch.tensor(Xb_aligned_pseudo, dtype=torch.float), Xb_aligned_pseudo_y.astype('int64')))
    Xb_aligned_pseudo_loader = torch.utils.data.DataLoader(Xb_aligned_pseudo_wrap, batch_size=args.batch_size, pin_memory=False, drop_last=False)
    ...
    for epoch in range(args.epochs):
        ...
        #####################################################################################
        # train()
        train_loss, train_acc, train_f1_score_micro, train_precision, train_recall, train_f1, train_roc_auc = train(num_classes, epoch, Xb_aligned_pseudo_loader, local_model_Xb, optimizer, args)
        ...
        cur_step = (epoch+1) * len(Xb_aligned_pseudo_loader)

        #####################################################################################
        # validation()
        test_losses, test_acc, test_f1_score_micro, test_precision, test_recall, test_f1, test_roc_auc = validation(num_classes, epoch, Xb_test_loader, local_model_Xb, args)
        ...
        if len(test_f1) == num_classes:
            if test_f1_score_micro > model_f1_score_micro:
                ...
                save_models(local_model_Xb, args.new_model_dir, args.dataset, name_str, '2-bbtune-pseudo', False, True)
    ...
    print('main_Xb_pseudo labels_semi-supervised local model-bbtune.py')
    print('local_model_Xb:{}'.format(local_model_Xb))


#####################################################################
def train(num_classes, epoch, Xb_aligned_pseudo_loader, local_model_Xb, optimizer, args):
    # train()
    ...
    cur_step = epoch * len(Xb_aligned_pseudo_loader)
    local_model_Xb.train()

    Xb_aligned_pseudo_iter = iter(Xb_aligned_pseudo_loader)
    
    for i in range(len(Xb_aligned_pseudo_loader)):
        Xb, labels = next(Xb_aligned_pseudo_iter)
        tran_Xb, trn_y = Xb.to(args.device), labels
        ...
        if i % args.report_freq == 0 or i == len(Xb_aligned_pseudo_loader) - 1:
            logging.info(
                "Train: [{:3d}/{}] Step {:03d}/{:03d} Loss {losses.avg:.3f} "
                "f1_score_micro ({f1_score_micro.avg:.6f})".format(
                    epoch + 1, args.epochs, i, len(Xb_aligned_pseudo_loader) - 1, losses=losses, f1_score_micro=f1_score_micro_train))
...
#####################################################################
def validation(num_classes, epoch, Xb_test_loader, local_model_Xb, args):
    # validation / test
    # validate()
    ...
    with torch.no_grad():
        ...
        for i in range(len(Xb_test_loader)):
            Xb, y = next(Xb_test_iter)
    ...
    logging.info(
        ...
        "f1_score_micro ({f1_score_micro.avg:.6f})".format(epoch + 1, args.epochs, i, len(Xb_test_loader) - 1, losses=losses, f1_score_micro=f1_score_micro_test))    
