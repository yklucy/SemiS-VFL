...
def main():
    ...
    for epoch in range(args.epochs):
        ...
        if len(test_f1) == num_classes:
            if test_f1_score_micro > model_f1_score_micro:
                ...
                save_models(local_model_Xa, args.new_model_dir, args.dataset, name_str, '1-bb', False, True)
    ...
    print("main_Xa_local_model_bb_load_globalmodelA.py")
...
def train(num_classes, epoch, Xa_local_loader, Xb_local_loader, local_model_Xa, optimizer, args):
    ...
    for i in range(len(Xa_local_loader)):
        ...
        if args.balanced == 1:
            ...
            tran_Xa, trn_y = balance_with_labels(Xa, y, num, args)
            print('After balancing:{}'.format(Counter(trn_y.numpy().tolist())))
            tran_Xa = tran_Xa.to(args.device)
        else:
