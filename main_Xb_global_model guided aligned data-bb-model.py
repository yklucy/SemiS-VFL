...
def main():
    ...
    if args.model_path !='':
        ...
        local_model_Xb = load_models(local_model_Xb, args.models, '2-bb', args, use_cross_model=False, use_local_model=True)
    ...
    for epoch in range(args.epochs):
        ...
        #####################################################################################
        # train()
        train_loss, train_acc, train_f1_score_micro, train_precision, train_recall, train_f1, train_roc_auc = train(num_classes, epoch, Xa_aligned_loader, Xb_aligned_loader, local_model_Xb, optimizer, args, global_model_Xa, global_model_Xb)
        ...
        cur_step = (epoch+1) * len(Xb_aligned_loader)
        ...
        if len(test_f1) == num_classes:
            if test_f1_score_micro > model_f1_score_micro:
                ...
                save_models(local_model_Xb, args.new_model_dir, args.dataset, name_str, 'tune-2-bb', False, True)
    ...
    print('main_Xb_global_model guided aligned data-bb-model.py')
    print('local_model_Xb:{}'.format(local_model_Xb))


#####################################################################
def train(num_classes, epoch, Xa_aligned_loader, Xb_aligned_loader, local_model_Xb, optimizer, args, global_model_Xa, global_model_Xb):
    # train()
    ...
    for i in range(len(Xa_aligned_loader)):
        ...
        Xb, labels = next(Xb_aligned_iter)
        ...
        with torch.no_grad():
            ...
            if args.dataset == 'NUSWIDE':
                global_model_num_features = global_model_Xb.cross_model.layer2[0].out_features
            else:
                global_model_num_features = global_model_Xb.cross_model.features[10].out_channels * global_model_Xb.cross_model.avgpool.output_size[0] * global_model_Xb.cross_model.avgpool.output_size[1]
            
            global_model_classifier_head_cat = nn.Linear(global_model_num_features*2, len(args.mul_classes))
            logits_g = global_model_classifier_head_cat(out) 
            loss_g = criterion(logits_g, target)
        
        logits = local_model_Xb.global_model_classifier_head_single(z_1) 
        
        loss = criterion(logits, target) * 0.3 + loss_g * 0.7
        ...
        if i % args.report_freq == 0 or i == len(Xb_aligned_loader) - 1:
            logging.info(
                "Train: [{:3d}/{}] Step {:03d}/{:03d} Loss {losses.avg:.3f} "
                "f1_score_micro ({f1_score_micro.avg:.6f})".format(
                    epoch + 1, args.epochs, i, len(Xb_aligned_loader) - 1, losses=losses, f1_score_micro=f1_score_micro_train))
