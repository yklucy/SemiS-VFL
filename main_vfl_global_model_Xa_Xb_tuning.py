...
def main():
    ...
    # nuswide_rate_Xa -> global
    args.nuswide_rate_Xa = 0.9
    args.nuswide_rate_Xb = 0.7

    args.emnist_rate_Xa = 0.6
    args.emnist_rate_Xb = 0.6

    args.cifar10_rate_Xa = 0.9
    args.cifar10_rate_Xb = 0.9
    ...
    ########################################################
    # load models
    # global_models for Xa
    if args.dataset == 'NUSWIDE':
        global_model_Xa = Party_A_Classification(input_shape_Xa, args, dropout_rate=0.3)
    else:
    ...
    # global_models for Xb
    if args.dataset == 'NUSWIDE':
        global_model_Xb = Party_B_Classification(input_shape_Xb, args, dropout_rate=0.3) 
    else:
    ...
    # load global models and local models
    if args.model_path !='':
        ...
        if args.dataset == 'NUSWIDE':
            local_model_Xa = load_models(local_model_Xa, args.models, '1-bb-tune', args, use_cross_model=False, use_local_model=True)
            local_model_Xb = load_models(local_model_Xb, args.models, '2-bbtune-pseudo', args, use_cross_model=False, use_local_model=True)            
        elif args.dataset == 'EMNIST':
            local_model_Xa = load_models(local_model_Xa, args.models, 1, args, use_cross_model=False, use_local_model=True)
            local_model_Xb = load_models(local_model_Xb, args.models, 2, args, use_cross_model=False, use_local_model=True)
        else:
            # CIFAR10
            local_model_Xa = load_models(local_model_Xa, args.models, 1, args, use_cross_model=False, use_local_model=True)
            local_model_Xb = load_models(local_model_Xb, args.models, 2, args, use_cross_model=False, use_local_model=True)
    ...
    # weights optimizer
    optimizer_Xa = torch.optim.SGD(global_model_Xa.parameters(), args.learning_rate, momentum=args.momentum, weight_decay=args.weight_decay)
    scheduler_Xa = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer_Xa, args.epochs)

    optimizer_Xb = torch.optim.SGD(global_model_Xb.parameters(), args.learning_rate, momentum=args.momentum, weight_decay=args.weight_decay)
    ...
    for epoch in range(args.epochs):
        ...
        #####################################################################################
        # train()
        train_loss, train_acc, train_f1_score_micro, train_precision, train_recall, train_f1, train_roc_auc = train(num_classes, epoch, Xa_aligned_loader, Xb_aligned_loader, optimizer_Xa, optimizer_Xb, criterion, global_model_Xa, local_model_Xa, global_model_Xb, local_model_Xb, args)
        ...
        # train_loader_aligned len:23    
        cur_step = (epoch+1) * len(Xa_aligned_loader)
        
        #####################################################################################
        # validation()
        test_losses, test_acc, test_f1_score_micro, test_precision, test_recall, test_f1, test_roc_auc = validation(num_classes, epoch, Xa_test_loader, Xb_test_loader, criterion, global_model_Xa, global_model_Xb, args)
        ...
        if len(test_f1) == num_classes:
            if test_f1_score_micro > model_f1_score_micro:
                ...
                save_models(global_model_Xa, args.new_model_dir, args.dataset, name_str, '1-tune', use_cross_model, use_local_model)
                save_models(global_model_Xb, args.new_model_dir, args.dataset, name_str, '2-tune', use_cross_model, use_local_model)
    ...
    print('main_vfl_global_model_Xa_Xb_tuning.py')
    print('model_Xa:{}'.format(global_model_Xa))
    print('model_Xb:{}'.format(global_model_Xb))

#####################################################################
def train(num_classes, epoch, Xa_aligned_loader, Xb_aligned_loader, optimizer_Xa, optimizer_Xb, criterion, global_model_Xa, local_model_Xa, global_model_Xb, local_model_Xb, args):
    ...
    # len(train_loader) = total train dataset samples / batch_size = 23
    ...
    global_model_Xa.train()
    global_model_Xb.train()
    ...
    for i in range(len(Xa_aligned_loader)):
        ...
        xa_g = global_model_Xa(tran_Xa)
        with torch.no_grad():
            xa_l = local_model_Xa(tran_Xa)
        # mean
        if args.dataset == 'NUSWIDE':
            z_0 = xa_g * args.nuswide_rate_Xa + xa_l * (1 - args.nuswide_rate_Xa)
        elif args.dataset == 'EMNIST':
            z_0 = xa_g * args.emnist_rate_Xa + xa_l * (1 - args.emnist_rate_Xa)
        else:
            z_0 = xa_g * args.cifar10_rate_Xa + xa_l * (1 - args.cifar10_rate_Xa)
        ...
        xb_g = global_model_Xb(tran_Xb)
        with torch.no_grad():
            xb_l = local_model_Xb(tran_Xb)

        if args.dataset == 'NUSWIDE':
            z_rest = xb_g * args.nuswide_rate_Xb + xb_l * (1 - args.nuswide_rate_Xb)
        elif args.dataset == 'EMNIST':
            z_rest = xb_g * args.emnist_rate_Xb + xb_l * (1 - args.emnist_rate_Xb)
        else:
            z_rest = xb_g * args.cifar10_rate_Xb + xb_l * (1 - args.cifar10_rate_Xb)

        z_1 = z_rest.detach().clone()
        z_1 = torch.autograd.Variable(z_1, requires_grad=True).to(args.device)
        out = torch.cat((z_0, z_1), dim=1)
        logits = global_model_Xa.global_model_classifier_head_cat(out)
        ...
        weights_gradients = torch.autograd.grad(z_rest, global_model_Xb.parameters(), grad_outputs=z_gradients, retain_graph=True, allow_unused=True)
        optimizer_Xa.zero_grad()
        loss.backward()

        if args.grad_clip > 0:
            # calculate the norm over all gradients together, input Tensor, return vector
            nn.utils.clip_grad_norm_(global_model_Xa.parameters(), args.grad_clip)
        ...
        for w, g in zip(global_model_Xb.parameters(), weights_gradients):
            if w.requires_grad:
                w.grad = g.detach()
        if args.grad_clip > 0:
            nn.utils.clip_grad_norm_(global_model_Xb.parameters(), args.grad_clip)
...
#####################################################################
def validation(num_classes, epoch, Xa_test_loader, Xb_test_loader, criterion, global_model_Xa, global_model_Xb, args):
    #####################################################################################
    # validation / test
    # validate()
    ...
    global_model_Xa.eval()
    global_model_Xb.eval()
